...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const userName = currentUser?.name?.trim() || currentUser?.email?.split('@')[0] || (userRole === 'admin' ? 'مدیر سیستم' : 'کاربر گرامی');
  const userInitial = (userName.charAt(0) || 'ک').toUpperCase();
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-2 sm:px-4 sm:py-2.5 rounded-2xl bg-purple-500/10 hover:bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-500/30 font-bold text-xs flex items-center gap-2.5 transition-all shadow-sm hover:scale-[1.02]"
                title="مشاهده حساب کاربری و داشبورد"
              >
                <div className="w-7 h-7 rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-600 text-white flex items-center justify-center font-bold text-xs shrink-0 shadow-sm">
                  {userInitial}
                </div>
                <div className="flex flex-col text-right">
                  <span className="max-w-[110px] sm:max-w-[170px] truncate leading-tight font-black text-slate-900 dark:text-white">{userName}</span>
